...
# 설치한 konlpy 외부라이브러리로부터 Hannaum 기능 사용하도록 설정
from konlpy.tag import Hannanum

# 문자열 수정을 위한 파이썬 기본 기능 추가
import re
...
analysis_text = (" ".join(myHannaum.nouns(replace_text)))

# 워드 클라우드를 생성하며, 생성된 워드 클라우드를 myWC 이름의 변수에 저장
# 워드 클라우드를 표시하는 단어가 한글일 경우, 파이썬에서 인식 불능 현상이 발생할 수 있기 때문에 글꼴을 강제 설정함
myWC = WordCloud(font_path="font/NanumGothic.ttf", background_color="white").generate(analysis_text)
